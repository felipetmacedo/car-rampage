...
void carregaTudo()
{
    ...
    cenario = LoadTexture("resources/cenario.png");
    title = LoadTexture("resources/titulo.png");
    carro = LoadTexture("resources/carro.png");
    mad = LoadTexture("resources/mad2.png");
    azul = LoadTexture("resources/carroazul.png");
    laranja = LoadTexture("resources/carrolaranja.png");
    vermelho = LoadTexture("resources/carrovermelho.png");
    engrenagem = LoadTexture("resources/engrenagem.png");
    gframe = LoadTexture("resources/gframe.png");
    botao = LoadTexture("resources/botao.png");
    vv = LoadTexture("resources/vv.png");
    va = LoadTexture("resources/va.png");
    vl = LoadTexture("resources/vl.png");

    // CARREGA A FONTE
    font = LoadFont("resources/komika.png");

    // CARREGA OS SONS
    transformacao = LoadSound("resources/transformacao.wav");
    ferramenta = LoadSound("resources/ferramenta.wav");
    boom = LoadSound("resources/explosao.wav");

    // CARREGA MUSICA
    music = LoadMusicStream("resources/tokyo2.ogg");
    madmodemusic = LoadMusicStream("resources/madmodemusic.ogg");
}
...
void desenhaTelaInicial(int *framescounter)
{
    ...
    DrawTextEx(font, "SCORES:", (Vector2){
                                    screenWidth / 2 - 600,
                                    screenHeight / 2 - 100,
                                },
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        case TITLE:
        {
            ...
            cenarioScrolling -= 2;
            if (cenarioScrolling <= -screenWidth)
                cenarioScrolling = 0;
                        ...
                        if (enemyType[i] < 3)
                        {
                            if (madMode)
                            {
                                if (enemyType[i] == 0)
                                    ...
                                    score += 150; // CARRO LARANJA
                                else if (enemyType[i] == 2)
                                    score += 300; // CARRO VERMELHO
                            ...
                            else
                            {
                                ...
                                if (score > hiscore)
                                    hiscore = score;
                                if (distance > hidistance)
                                    hidistance = distance + 1;
                            }
                        }
                        else
        ...
        case TITLE:
        {
            desenhaTelaInicial(&framesCounter);
        }
        break;
        case GAMEPLAY:
        {
        ...
        case ENDING:
        {
            desenhaTelaFinal(&score, &distance, &hiscore, &hidistance, &framesCounter);
        }
        break;
        default:
            break;
